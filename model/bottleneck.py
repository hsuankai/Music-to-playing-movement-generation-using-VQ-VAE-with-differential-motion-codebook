...
class BottleneckBlock(nn.Module):
    ...
    def init_k(self, x):
        ...
        self.k = _k_rand # 512 x 512 first dim is num_embeddings sencond dim is embedding_dim
    ...
    def update_k(self, x, x_l):
        ...
        with t.no_grad():
            ...
            _k_elem = x_l_onehot.sum(dim=-1)  # k_bins calculate embedding desity
            y = self._tile(x)
            _k_rand = y[t.randperm(y.shape[0])][:k_bins]
            ...
            # Update centres
            old_k = self.k
            self.k_sum = mu * self.k_sum + (1. - mu) * _k_sum  # k_bins, w
    ...
    def forward(self, x, update_k=True):
        ...
        x_l, fit = self.quantise(x)
        x_d = self.dequantise(x_l) # equals to quantized
